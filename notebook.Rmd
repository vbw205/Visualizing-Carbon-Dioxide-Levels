...
head(noaa_data)
noaa_viz <- ggplot(data = noaa_data, aes(x = Age_yrBP, y = CO2_ppmv)) + geom_line() + labs(title = "Carbon Dioxide Levels From 8,000 to 126 Years BP", subtitle = "From World Data Center for Paleoclimatology and NOAA Paleoclimatology Program", x = "Years Before Today (0 = 1950)", y = "Carbon Dioxide Level (Parts Per Million)") + scale_x_reverse(lim = c(800000, 0))
noaa_viz
```
...
head(iac_data)
iac_viz <- ggplot(data = iac_data, aes(x = year, y = data_mean_global)) + geom_line() + labs(title = "Carbon Dioxide Levels over Time", subtitle = "From Institute for Atmospheric and Climate Science (IAC).", x = 'Year', y = "Carbon Dioxide Levels (Parts Per Million)") + geom_hline(aes(yintercept = millennia_max, linetype = "Historical CO2 Peak before 1950"))
iac_viz
```
